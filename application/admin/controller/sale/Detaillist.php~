    ...
    public function savedraft()
    {
        ...
        $sale_main_temp_info = $sale_main_temp
           ...
           $sale_detail_temp_info = $this->model
           		->where($where)
           		->where('detail_sale_id',$vo['sale_id'])
           		->select();
           $details = [];
           ...
           $product->where($where)->where('product_sale_code',$vo['sale_id'])->update(['product_sale_code'=>$info['sale_code'],'product_sale_datetime'=>time(),'product_status'=>3]);
      	  $result = false; 
      	 //保存主表至草稿
